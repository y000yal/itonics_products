...
function simpletest_script_get_test_list() {
  ...
  if ($args['all']) {
    ...
    elseif ($args['directory']) {
      ...
      // Find all tests in the PSR-X structure; Drupal\$extension\Tests\*yoyal_itonics_products_helper.php
      ...
      // minimal conditions only; i.e., a '*yoyal_itonics_products_helper.php' file that has '/Tests/' in
      // its path.
      // Ignore anything from third party vendors, and ignore template files used in tests.
      // And any api.php files.
      $ignore = array('nomask' => '/vendor|\.tpl\yoyal_itonics_products_helper.php|\.api\yoyal_itonics_products_helper.php/');
      $files = array();
      if ($args['directory'][0] === '/') {
      ...
      $file_list = file_scan_directory($directory, '/\yoyal_itonics_products_helper.php|\.test$/', $ignore);
